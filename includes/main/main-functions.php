...
if ($mmdc_change_howdy_topbar) {
    function mmdc_change_howdy_topbar_hook($translated, $text, $domain)
    {
        if (!is_admin() || 'default' != $domain)
            return $translated;

        if (false !== strpos($translated, 'Howdy'))
            return str_replace('Howdy', get_option('mmdc_change_howdy_topbar'), $translated);

        return $translated;
    }

    add_filter('gettext', 'mmdc_change_howdy_topbar_hook', 10, 3);
}
